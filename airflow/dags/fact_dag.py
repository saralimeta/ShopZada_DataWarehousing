...
def order_with_merchant_cleaning():
    # loading data frame
    df_order_with_merchant1 = pd.read_parquet("/opt/airflow/departments/Enterprise Department/order_with_merchant_data1.parquet")
    df_order_with_merchant2 = pd.read_parquet("/opt/airflow/departments/Enterprise Department/order_with_merchant_data2.parquet")
    df_order_with_merchant3 = pd.read_csv("/opt/airflow/departments/Enterprise Department/order_with_merchant_data3.csv")
    ...
    merged_df_order_with_merchant.to_parquet("/opt/airflow/dimensions/Transaction Fact Table/order_with_merchant_data_cleaned.parquet")
    print("Successfully cleaned the order_with_merchant")

def fact_table():
    # loading data frame
    df_cleaned_lineData = pd.read_parquet(("/opt/airflow/dimensions/Order Dimension/line_product_and_prices_cleaned.parquet"))
    df_order_merchant = pd.read_parquet("/opt/airflow/dimensions/Transaction Fact Table/order_with_merchant_data_cleaned.parquet")
    df_order_data = pd.read_parquet("/opt/airflow/dimensions/Order Dimension/cleaned_order_data.parquet")
    df_transac_campaign = pd.read_parquet("/opt/airflow/dimensions/Order Dimension/transactional_campaign_data_standardized.parquet")
    ...
    # merged_lineData_orderMerchant_orderData_campaign = merged_lineData_orderMerchant_orderData_campaign.drop('PRODUCT_NAME', axis = 1)
    ...
    merged_lineData_orderMerchant_orderData_campaign.to_parquet("/opt/airflow/dimensions/Dimensional Model/fact_table.parquet")


def fact_table_to_db():

# Step 1: Read Parquet File
    parquet_file_path = "/opt/airflow/dimensions/Dimensional Model/fact_table.parquet"
